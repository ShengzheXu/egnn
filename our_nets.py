...
class SingleTaskNet(nn.Module):
    def __init__(self, dim_in, dim_out, dim_window=1, mask_mode=None, encoder_arch=None, decoder_arch=None, model_tag=None):
        ...
        if encoder_arch is not None:
            ...
            curr_in = 168
        assert decoder_arch[0] in ['gmm', 'softmax', 'sigmoid'], "Unknown Decoder Type"
        ...
        elif self.decoder_type == 'softmax': #softmax
            self.dec_network = self.make_decs(curr_in, dim_out, hidden_dim=decoder_arch[1], is_onehot=True)
        elif self.decoder_type == 'sigmoid':
            self.multilabel_network = self.make_mls(curr_in, dim_out, hidden_dim=decoder_arch[1])
        else:
            input('decoder error')
    ...
    def make_mls(self, input_dim, output_dim, hidden_dim=100):
        if is_onehot:
            ml_layer = nn.Sequential(
                #nn.Linear(input_num, hidden_num),
                #nn.ReLU(inplace=True),
                #nn.Linear(hidden_num, output_num),
                nn.Linear(input_dim, output_dim),
