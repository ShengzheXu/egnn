...
class NetworkTrafficTransformer(object):
    ...
    def _flag_dummy(self, flg):
        # UAPRSF
        rt = [0 if x == '.' else 1 for x in flg]
        rt2 = []
        for i in rt:
            rt2 += [1, 0] if i == 0 else [0, 1]
        return rt2
    
    def _rev_flag_dummy(self, flg_list):
        UAPRSF = 'UAPRSF'
        st = ''
        for i in range(6):
            if flg_list[i] == 0:
    ...
    def transfer(self, df):
        ...
        for index, row in df.iterrows():
            ...
            flg_list = self._flag_dummy(row['flg'])
            
            line += flg_list
            label_line += flg_list
            # print(len(line), len(label_line))
            # input()
